...
class KPHourglassUnet(nn.Module):
    ...
    def __init__(self, block_expansion, in_features, reshape_features, reshape_depth, num_blocks=3, max_features=256):
        ...
        for i in range(num_blocks):
            self.down_blocks.append(DownBlock2d(in_features if i == 0 else min(max_features, block_expansion * (2 ** i)),
                                                                   min(max_features, block_expansion * (2 ** (i + 1))),
                                                                   kernel_size=3, padding=1))
            in_features_list.append(min(max_features, block_expansion * (2 ** (i + 1))))
        ...
        in_features_list.pop()
        self.up_blocks = nn.ModuleList()
        for i in range(num_blocks):
            in_filters = min(max_features, block_expansion * (2 ** (num_blocks - i)))
            if i > 0:
                in_filters += in_features_list.pop()
            out_filters = min(max_features, block_expansion * (2 ** (num_blocks - i - 1)))
            self.up_blocks.append(UpBlock3d(in_filters, out_filters, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.up_blocks):
            if i > 0:
                sub = features.pop().unsqueeze(2).repeat(1, 1, self.reshape_depth, 1, 1)
                out = torch.cat([out, sub], dim=1)
            out = layer(out)
        return out
