...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, kp_extractor, generator, discriminator, train_params, estimate_jacobian=True):
    ...
    def forward(self, x):
        ...
        R_source, t_source, c_source = x['source_mesh']['R'], x['source_mesh']['t'], x['source_mesh']['c']
        R_driving, t_driving, c_source = x['driving_mesh']['R'], x['driving_mesh']['t'], x['driving_mesh']['c']
        kp_source = self.kp_extractor(x['source'], R_source, t_source, c_source)     # {'value': value, 'jacobian': jacobian}   
        kp_driving = self.kp_extractor(x['driving'], R_driving, t_driving, c_source)     # {'value': value, 'jacobian': jacobian}   
